...
def main():
    ...
    last_line = lines[-1]
    if ":" in last_line:
        last_line = last_line[last_line.index(":") + 1:].strip()
    last = last_line.replace(",", " ").split()

    is_correct = True
    if last[0] != "beq":
        is_correct = False
    if last[1] != "zero" and last[1] != "x0":
        is_correct = False
    if last[2] != "zero" and last[2] != "x0":
        is_correct = False
    if is_correct == False:
        print(f"Error at line {len(lines)}: Last instruction must be virtual halt: beq zero, zero, 0")
        return
